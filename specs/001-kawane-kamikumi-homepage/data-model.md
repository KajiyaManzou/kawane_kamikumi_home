...
**表示ルール**: トップページに最新1件をハイライト表示。過去分は一覧ページ（`/circulars/`）に表示。
...
**表示ルール**: トップページに最新5件表示。タイトルは元サイトへの外部リンク（別タブ）。
...
| エンティティ | 入力パス | 出力URL |
|-------------|---------|---------|
| お知らせ（最新5件） | `content/notices/*.md` | `/`（トップページ内セクション） |
| お知らせ（アーカイブ） | `content/notices/*.md` | `/archives/` |
| 回覧板（最新1件） | `content/circulars/*.md` | `/`（トップページ内セクション） |
